...
if _FROZEN:
    _psd_bin = APP_DIR / "bin" / "houmi-psd-cli.exe"
    if not _psd_bin.exists():
        _psd_bin = APP_DIR / "bin" / "manga-psd-cli.exe"
    PSD_CLI_PATH = _psd_bin
else:
    _psd_dev = BASE_DIR.parent / "houmi-psd-cli" / "target" / "release" / "houmi-psd-cli.exe"
    if not _psd_dev.exists():
        _psd_dev = BASE_DIR.parent / "manga-psd-cli" / "target" / "release" / "manga-psd-cli.exe"
    PSD_CLI_PATH = _psd_dev
