...
def _ask_folder_dialog(title: str = "เลือกโฟลเดอร์", initialdir: Optional[str] = None) -> Optional[str]:
    # 1. Native Windows PowerShell Folder Dialog (100% thread-safe on Windows)
    if sys.platform == "win32":
        try:
            import subprocess
            init_arg = initialdir.replace("'", "''") if initialdir else ""
            title_arg = title.replace("'", "''")
            ps_script = f"""
            Add-Type -AssemblyName System.Windows.Forms
            $f = New-Object System.Windows.Forms.FolderBrowserDialog
            $f.Description = '{title_arg}'
            $f.ShowNewFolderButton = $true
            if ('{init_arg}' -ne '' -and (Test-Path '{init_arg}')) {{
                $f.SelectedPath = '{init_arg}'
            }}
            ...
            logging.getLogger("houmi-projects").warning("PowerShell Folder dialog failed: %s", exc)
