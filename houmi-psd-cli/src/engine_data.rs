...
pub struct TextEngineSpec {
    ...
    pub anti_alias: PsdAntiAlias,
    pub orientation: TextOrientation,
    pub justification: TextJustification,
    /// Outline width in px
    pub stroke_width: f64,
    pub stroke_color: [u8; 4],
    /// Absolute leading in px (0 = auto)
    pub leading: f64,
    /// Tracking in thousandths of an em
    pub tracking: i32,
    /// Local paragraph text box [left, top, right, bottom].
    ///
    /// Photoshop balloon text is area/paragraph text (ShapeType 1), not point
    /// text.  Supplying the canonical TypesettingSpec box prevents Photoshop
    /// from converting and reflowing the layer on the first edit/transform.
...
pub fn encode_engine_data(spec: &TextEngineSpec) -> Vec<u8> {
    ...
    let shape_type = if spec.box_bounds.is_some() { 1 } else { 0 };
    let photoshop_shape_geometry = if let Some(bounds) = spec.box_bounds {
        (
            "BoxBounds".to_string(),
            EngineValue::Array(bounds.into_iter().map(EngineValue::Float).collect()),
        )
    } else {
        (
            "PointBase".to_string(),
            EngineValue::Array(vec![EngineValue::Float(0.0), EngineValue::Float(0.0)]),
        )
    };
    ...
    let root = EngineValue::Dict(vec![
        (
            ...
            EngineValue::Dict(vec![
                ...
                // Keep EngineData and the TySh AntA descriptor identical.
                (
                    "AntiAlias".to_string(),
                    EngineValue::Int(spec.anti_alias.engine_value()),
                ),
                (
                        ...
                        (
                            ...
                            EngineValue::Dict(vec![
                                ...
                                (
                                    "Children".to_string(),
                                    EngineValue::Array(vec![EngineValue::Dict(vec![
                                        ("ShapeType".to_string(), EngineValue::Int(shape_type)),
                                        ...
                                        (
                                            ...
                                            EngineValue::Dict(vec![(
                                                ...
                                                EngineValue::Dict(vec![
                                                    (
                                                        "ShapeType".to_string(),
                                                        EngineValue::Int(shape_type),
                                                    ),
                                                    photoshop_shape_geometry.clone(),
                                                    (
                                                        "Base".to_string(),
                                                        EngineValue::Dict(vec![
                                                            (
...
fn paragraph_properties(justification: TextJustification) -> Vec<(String, EngineValue)> {
    vec![
        ...
        ("AutoHyphenate".to_string(), EngineValue::Bool(false)),
...
fn base_style_sheet(font_index: i32) -> Vec<(String, EngineValue)> {
    vec![
        ...
        ("OutlineWidth".to_string(), EngineValue::Float(0.0)),
...
fn style_run_sheet(spec: &TextEngineSpec, font_index: i32) -> Vec<(String, EngineValue)> {
    let stroke_on = spec.stroke_width > 0.05 && spec.stroke_color != spec.color;
    let auto_leading = spec.leading <= 0.0;
    vec![
        ("Font".to_string(), EngineValue::Int(font_index)),
        ...
        ("AutoLeading".to_string(), EngineValue::Bool(auto_leading)),
        (
            "Leading".to_string(),
            EngineValue::Float(if auto_leading { 0.0 } else { spec.leading }),
        ),
        ("Tracking".to_string(), EngineValue::Int(spec.tracking)),
        ("AutoKerning".to_string(), EngineValue::Bool(true)),
        ("Kerning".to_string(), EngineValue::Int(0)),
        ("FontBaseline".to_string(), EngineValue::Int(0)),
        ("FontCaps".to_string(), EngineValue::Int(0)),
        ...
        (
            "StrokeColor".to_string(),
            EngineValue::Dict(color_type_values(spec.stroke_color)),
        ),
        ("StrokeFlag".to_string(), EngineValue::Bool(stroke_on)),
        (
            "OutlineWidth".to_string(),
            EngineValue::Float(if stroke_on { spec.stroke_width } else { 0.0 }),
        ),
        ("FillFlag".to_string(), EngineValue::Bool(true)),
        ("FillFirst".to_string(), EngineValue::Bool(true)),
    ]
...
fn font_set(font_name: &str) -> Vec<EngineValue> {
    vec![font_descriptor(font_name)]
}

fn font_descriptor(name: &str) -> EngineValue {
    let family_name = if name == "TFPhethai" {
        "TF Phethai"
    ...
    EngineValue::Dict(vec![
        ...
        (
            ...
            EngineValue::String(family_name.to_string()),
        ),
        (
            "StyleName".to_string(),
            EngineValue::String("Regular".to_string()),
        ),
        ("Script".to_string(), EngineValue::Int(0)),
        ("FontType".to_string(), EngineValue::Int(1)),
...
fn color_type_values(color: [u8; 4]) -> Vec<(String, EngineValue)> {
    vec![
        ("Type".to_string(), EngineValue::Int(0)),
        (
            "Values".to_string(),
            EngineValue::Array(vec![
...
mod tests {
    ...
    fn engine_data_contains_expected_sections_and_utf16_text() {
        let bytes = encode_engine_data(&TextEngineSpec {
            ...
            faux_bold: true,
            faux_italic: false,
            anti_alias: PsdAntiAlias::Smooth,
            ...
            leading: 0.0,
            tracking: 0,
            box_bounds: Some([0.0, 0.0, 240.0, 120.0]),
        });
    ...
    fn engine_data_keeps_float_tokens_for_font_size_and_transforms() {
        ...
        for expected in [
            b"/ShapeType 1".as_slice(),
            b"/BoxBounds [ 0.0 0.0 240.0 120.0 ]".as_slice(),
            b"/AutoLeading false".as_slice(),
            b"/Leading 16.0".as_slice(),
            b"/Tracking 20".as_slice(),
            b"/StrokeFlag true".as_slice(),
            b"/OutlineWidth 1.0".as_slice(),
        ] {
            assert!(
                bytes
                    .windows(expected.len())
                    .any(|window| window == expected),
                "missing engine-data token: {}",
