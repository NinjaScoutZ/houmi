...
fn text_layer(
    ...
    let text = match mode {
        ...
        TextLayerMode::Editable => {
            ...
            // Keep the affine transform limited to document placement and
            // rotation. Vertical alignment belongs in the local paragraph box;
            // translating both the transform and a full-height BoxBounds makes
            // Photoshop see two conflicting geometries and request Transform
            // recomposition. Keeping the bottom edge fixed also lets importers
            // reconstruct the canonical outer balloon height losslessly.
            let bounds = [
                left as f64,
                top as f64 + vertical_offset,
                (left + inner_width) as f64,
                (top + inner_height) as f64,
            ];
...
mod tests {
    ...
    use super::{
        ...
        infer_font_name, infer_orientation, is_probably_latin, place_on_canvas,
    ...
    fn editable_text_geometry_uses_canonical_engine_canvas() {
        ...
        assert_eq!(metadata.bounds, [0.0, 48.0, 252.0, 120.0]);
