# =========================================================
#  HOUMI STUDIO - POWERSHELL ONLINE PATCH PUBLISHER v0.4.2
# =========================================================
...
Write-Host "  HOUMI STUDIO - PUBLISH ONLINE PATCH v0.4.2 Hotfix" -ForegroundColor Yellow
Write-Host "==================================================" -ForegroundColor Yellow
...
# 3. Create update manifest for v0.4.2
$manifest = [ordered]@{
    latest_version   = "0.4.2"
    patch_notes      = "v0.4.2 Hotfix Release - Resolved PSD export text style italic bug (fixed default thought template & cleaned database italic flags)"
...
Write-Host "✅ [PowerShell] Updated manifest files to v0.4.2!" -ForegroundColor Green

# 4. Notify dev_patch_service in backend
Push-Location "E:\houmi\backend"
.venv\Scripts\python.exe -c "import sys; sys.path.insert(0, '.'); from app.services.dev_patch_service import record_dev_patch; record_dev_patch({'is_customer_release': True, 'customer_version': '0.4.2', 'changes': [{'category': 'Fixed', 'description': 'Fixed PSD export text style italic bug for thought template and database italic flags'}]})"
Pop-Location

Write-Host "🚀 Patch v0.4.2 published successfully!" -ForegroundColor Green
