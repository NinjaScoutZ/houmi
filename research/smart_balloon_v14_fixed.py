"""Smart Balloon V14 Fixed: Pure Raw Contour Feature Preservation with Waist-Bridged Healing.

Located and executed exclusively inside e:\\houmi\\research\\

Architecture:
1. Extracts 100% RAW binary white mask (zero morphological degradation).
2. Finds the exact physical Left and Right Waist Pinch Points on the raw line art.
3. Balloon 1 takes the RAW top envelope and bridges a natural bottom arc.
4. Balloon 2 takes the RAW bottom envelope (preserving all spikes, corners, and speech tails) and bridges a natural top arc.
5. Inpaints solid pure white and renders high-definition 4-panel verification.
"""
...
OUTPUT_DIR = RESEARCH_DIR / "v14_fixed_previews"
...
def process_page_universal_fixed(
    page_img: np.ndarray,
    blk1: dict,
    blk2: dict,
    sample_name: str
    ...
    for b_idx in (1, 2):
        ...
        print(f"[{sample_name}] Balloon {b_idx}: {b_type} {meta}")
    ...
    # 4. Pure Raw Shape-Preserving Reconstruction
    poly1 = reconstruct_raw_balloon_top(raw_combined_cnt, left_waist, right_waist, adjusted_blocks[0]["bbox"])
    poly2 = reconstruct_raw_balloon_bottom(raw_combined_cnt, left_waist, right_waist, adjusted_blocks[1]["bbox"], tail_tip2)
    ...
    balloon_data = [
        {"id": 1, "type": type1, "text": adjusted_blocks[0]["text"], "contour": poly1, "tail_tip": None, "mask": mask1},
        {"id": 2, "type": type2, "text": adjusted_blocks[1]["text"], "contour": poly2, "tail_tip": tail_tip2, "mask": mask2},
    ]
    ...
    p2 = crop.copy()
    ov2 = p2.copy()
    c_color1 = COLOR_MAP.get(type1, (200, 200, 200))
    c_color2 = COLOR_MAP.get(type2, (200, 200, 200))
    ov2[raw_lbl == 1] = [int(c * 0.7 + 50) for c in c_color1]
    ov2[raw_lbl == 2] = [int(c * 0.7 + 50) for c in c_color2]
    cv2.addWeighted(ov2, 0.45, p2, 0.55, 0, p2)
    cv2.circle(p2, text_centers[0], 6, c_color1, -1)
    cv2.circle(p2, text_centers[1], 6, c_color2, -1)
    ...
    for b_item in balloon_data:
        ...
        cv2.drawContours(p3, [b_item["contour"]], -1, b_col, 2)
        if b_item["tail_tip"] is not None:
            cv2.circle(p3, b_item["tail_tip"], 7, (0, 0, 255), -1)
    cv2.putText(p3, "3. 2 CLOSED ORGANIC LOOPS", (15, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2)
    
    p4 = cleaned.copy()
    for b_item in balloon_data:
        b_col = COLOR_MAP.get(b_item["type"], (0, 255, 0))
        cv2.drawContours(p4, [b_item["contour"]], -1, b_col, 2)
        rx, ry, rw, rh = b_item["bbox"]
        cx, cy = b_item["center"]
        cv2.line(p4, (rx, cy), (rx + rw, cy), (255, 150, 0), 3)
        cv2.circle(p4, (cx, cy), 6, (255, 100, 0), -1)
    cv2.putText(p4, "4. CLEANED & TRUE CENTERS", (15, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2)
    
    out_img = np.hstack([p1, p2, p3, p4])
    out_file = OUTPUT_DIR / f"v14_fixed_{sample_name}.png"
...
def main():
    print("=== STARTING SMART BALLOON V14 FIXED (PURE FEATURE PRESERVATION) ===")
    ...
    process_page_universal_fixed(p10_img, p10_data["text_blocks"][0], p10_data["text_blocks"][1], "page10_spiky")
    
    # 2. Page 20 (ANGULAR)
    p20_data = [p for p in proj_112["pages"] if p["page_number"] == 20][0]
    p20_img = load_image(CHAPTER_112_DIR / "20.jpg")
    process_page_universal_fixed(p20_img, p20_data["text_blocks"][0], p20_data["text_blocks"][1], "page20_angular")
    ...
    process_page_universal_fixed(p3_img, p3_data["text_blocks"][1], p3_data["text_blocks"][2], "page03_smooth_oval")
    
    # 4. Page 11 (RECTANGULAR)
    p11_data = [p for p in proj_112["pages"] if p["page_number"] == 11][0]
    p11_img = load_image(CHAPTER_112_DIR / "11.jpg")
    process_page_universal_fixed(p11_img, p11_data["text_blocks"][0], p11_data["text_blocks"][1], "page11_rectangular")
    
    print("\nSmart Balloon V14 Fixed testing completed successfully!")
    return 0
