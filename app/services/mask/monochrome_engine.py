...
def generate_monochrome_flat_text_mask(
    ...
    center_crop = gray[cy0:cy1, cx0:cx1] if (cy1 > cy0 and cx1 > cx0) else gray
    center_brightness = float(np.median(center_crop))

    # Case A: True dark background (both center and overall are dark) -> Extract bright/glowing text
    if background_gray < 110 and center_brightness < 130:
